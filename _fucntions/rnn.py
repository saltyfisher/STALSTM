...
def STALSTMCell(input, hidden, w_ih, w_hh, w_fc1, w_fc2, b_ih=None, b_hh=None, b_fc1=None, b_fc2=None):
    #input[0]:xt-1    input[1]:xt
    hy, cy = LSTMCell(input[1], (hidden[0], hidden[1]), w_ih, w_hh, b_ih, b_hh)

    fc1 = F.linear(torch.cat((hy, input[0]), 1), w_fc1, b_fc1)
    fc1 = torch.tanh(fc1)
    fc2 = F.linear(fc1, w_fc2, b_fc2)
    fc2 = torch.exp(fc2)
    sum = torch.sum(fc2, 1)
    sum = sum.reshape(-1, 1)
    fc2 = fc2 / sum

    sorted_fc2, indices = torch.sort(fc2, 1)
    clip = int(sorted_fc2.size(1) / 5)

    for i in range(4):
        mean = torch.mean(sorted_fc2[:, i*clip:(i+1)*clip], 1)
        mean = mean.reshape(-1, 1)
        sorted_fc2[:, i*clip:(i+1)*clip] = mean
    mean = torch.mean(sorted_fc2[:, i * clip:(i + 1) * clip], 1)
    mean = mean.reshape(-1, 1)
    sorted_fc2[:, 4*clip:] = mean

    input_w = sorted_fc2.new_zeros(sorted_fc2.size(0), sorted_fc2.size(1)*3)
    for i in range(indices.size(0)):
        for j in range(indices.size(1)):
            input_w[i, indices[i, j]*3:indices[i, j]*3+3] = sorted_fc2[i, j]
    
    return hy, cy, input_w
...
def Recurrent(inner, reverse=False):
    def forward(input, hidden, weight, batch_sizes):
        ...
        if inner.__name__ == 'STALSTMCell':
            for i in steps:
                if i == 0:
                    continue
                else:
                    hidden = inner([input[i-1], input[i]], hidden, *weight)
                # hack to handle STALSTM
                    output.append(hidden[2] if isinstance(hidden, tuple) else hidden)
            if reverse:
                output.reverse()
            output = torch.cat(output, 0).view(input.size(0)-1, *output[0].size())
